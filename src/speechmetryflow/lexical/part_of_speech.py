...
class POS:
    ...
    def proportions(self):
        for tag, count in self.counts.items():
            self.metrics[f'pos_prop_{tag}'] = count / self.len_text if self.len_text > 0 else None
    ...
    def count_open_class_words(self):
        ...
        self.metrics['pos_prop_open_words'] = num / self.len_text if self.len_text > 0 else None
    ...
    def count_closed_class_words(self):
        ...
        self.metrics['pos_prop_closed_words'] = num / self.len_text if self.len_text > 0 else None
    ...
    def count_verb_inflection(self):
        ...
        num = sum(1 for token in self.doc if token.pos_ == 'VERB' and token.tag_ != 'VB')
        self.metrics['pos_count_verb_inflection'] = num
        self.metrics['pos_prop_verb_inflection'] = num / self.len_text if self.len_text > 0 else None
    ...
    def count_verb_gerondif(self):
        ...
        self.metrics['pos_prop_verb_gerondif'] = num / self.len_text if self.len_text > 0 else None
    ...
    def compute_ratios(self):
        ...
        # Calcul des ratios
        self.metrics['PRON/(NOUN+PRON)'] = n_pron / (n_noun + n_pron) if (n_noun + n_pron) > 0 else None
        self.metrics['NOUN/(NOUN+PRON)'] = n_noun / (n_noun + n_pron) if (n_noun + n_pron) > 0 else None
        self.metrics['NOUN/(NOUN+VERB)'] = n_noun / (n_noun + n_verb) if (n_noun + n_verb) > 0 else None
        self.metrics['VERB/(NOUN+VERB)'] = n_verb / (n_noun + n_verb) if (n_noun + n_verb) > 0 else None
        self.metrics['VERB_INFLECTION/VERB'] = n_inflection / n_verb if n_verb > 0 else None
