'''
Pragmatic characteristics
'''

# Uncertainty words
uncertainty_words = {
    'en': {'think', 'look', 'like', 'kind', 'seem', 'maybe', 'can', 'something'},
    'fr': {'penser', 'sembler', 'comme', 'genre', 'peut-être', 'peut', 'quelque', 'chose'},
}

# Formulaic expressions
formulaic_expressions = {
    'en': {'well', 'so', 'i guess', 'you know', 'as it is', 'as it were'},
    'fr': {'bien', 'alors', 'je suppose', 'tu sais', 'comme il est', 'comme il serait'},
}

# Modal expressions
modal_expressions = {
    'en': {'i think', 'in my opinion', 'of course', 'naturally', 'unsure', 'likely', 'could be that', 'unfortunately', 'surely'},
    'fr': {'je pense', 'à mon avis', 'bien sûr', 'naturellement', 'incertain', 'probablement', 'ça pourrait être que', 'malheureusement', 'sûrement'},
}

# Filler expressions
filler_expressions = {
    'en': {'you know', 'i mean'},
    'fr': {'tu sais', 'je veux dire'},
}

# Words indicating lexical access difficulties
difficulty_words = {
    'en': {'know', 'remember', 'unable'},
    'fr': {'savoir', 'se souvenir', 'incapable'}  
}